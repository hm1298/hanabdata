...
from hanabdata.tools.io.read import read_csv
...
if __name__ == "__main__":
    for i in range(2, 7):
        if i > 2:
            break
        table = read_csv(f"./data/processed/ratings/variants_iter_2p_bare10.csv")
        table.pop(0)
        ...
        print(f"For teams with 2 players, variants with the given simple suit have average rating:")
        for j, rating_pair in enumerate(lb_simple):
            if len(rating_pair[0]) > 7:
                print(f"{j + 1}\t{rating_pair[0]}\t{rating_pair[1]:.4f}")
            else:
                print(f"{j + 1}\t{rating_pair[0]}\t\t{rating_pair[1]:.4f}")
        print(f"For teams with 2 players, variants with the given dark suit have average rating:")
